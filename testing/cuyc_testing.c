    ...
    switch (msg) {
        case WM_CHAR:
            if (wParam == VK_BACK) {
                ...
                if (lineCount < MAX_LINES) {
                    sprintf(lines[lineCount++], "> %s", inputBuffer);
                    inputBuffer[0] = '\0';
        ...
        case WM_PAINT: {
            ...
            for (int i = 0; i < lineCount; i++) {
                TextOut(hdc, 10, y, lines[i], strlen(lines[i]));
                y += 20;
            }

            char inputLine[MAX_CHARS + 3];
            sprintf(inputLine, "> %s", inputBuffer);
            TextOut(hdc, 10, y, inputLine, strlen(inputLine));
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrev, LPSTR lpCmdLine, int nCmdShow) {
    const char *CLASS_NAME = "RawTerminalWindow";
    ...
    HWND hwnd = CreateWindow(CLASS_NAME, "Custom Terminal", WS_OVERLAPPEDWINDOW,
